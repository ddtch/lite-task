/**
 * Shared MCP tool definitions + direct-DB tool handler.
 *
 * Used by both transports that talk to the DB directly:
 *  - mcp/server.ts  → stdio (Claude Desktop, local)
 *  - routes/mcp.ts  → Streamable HTTP endpoint at /mcp (Cursor, remote)
 *
 * mcp/http-client.ts keeps its own copy of the definitions because it
 * proxies through the REST API instead of the DB.
 *
 * No MCP SDK imports here — routes/mcp.ts must lazy-load the SDK to avoid
 * a Vite SSR crash, so this module stays SDK-free.
 */
...
export const TOOLS = [
  ...
  },
  {
    name: "update_event",
    description:
      "Update a calendar event, note, or reminder. Only provided fields are updated. Pass event_date to move the entry to another day and event_time to change its time — rescheduling re-arms the Telegram/phone notifications.",
    inputSchema: {
      type: "object",
      properties: {
